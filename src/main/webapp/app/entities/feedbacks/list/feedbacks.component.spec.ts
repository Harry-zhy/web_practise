import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute } from '@angular/router';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';

import { FeedbacksService } from '../service/feedbacks.service';

import { FeedbacksComponent } from './feedbacks.component';

describe('Feedbacks Management Component', () => {
  let comp: FeedbacksComponent;
  let fixture: ComponentFixture<FeedbacksComponent>;
  let service: FeedbacksService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'feedbacks', component: FeedbacksComponent }]), HttpClientTestingModule],
      declarations: [FeedbacksComponent],
      providers: [
        {
          provide: ActivatedRoute,
          useValue: {
            data: of({
              defaultSort: 'id,asc',
            }),
            queryParamMap: of(
              jest.requireActual('@angular/router').convertToParamMap({
                page: '1',
                size: '1',
                sort: 'id,desc',
              })
            ),
            snapshot: { queryParams: {} },
          },
        },
      ],
    })
      .overrideTemplate(FeedbacksComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(FeedbacksComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(FeedbacksService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.feedbacks?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to feedbacksService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getFeedbacksIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getFeedbacksIdentifier).toHaveBeenCalledWith(entity);
      expect(id).toBe(entity.id);
    });
  });
});
